...
const BOTCODE = '01743'; // const HOST = 'host.docker.internal';
...
class Client {
  ...
  init() {
    ...
    client.socket.on('close', () => {
      ...
      if (client.intervalTimer != null) clearInterval(client.intervalTimer);
      client.reconnect();
    });
    ...
    client.socket.on('connect', () => {
      ...
      client.intervalTimer = setInterval(() => {
        client.getData(`${_config.BASE_JSON_URL}${client.gameHash}_game.json`);
      }, 2000);
    });
    client.socket.on('error', error => {
      console.log("Error ==== ", error);
      if (client.intervalTimer != null) clearInterval(client.intervalTimer); // client.socket.removeAllListeners();
    });
  }
  ...
  destroy() {
    let client = this;
    if (client.dataSubscription != null) client.dataSubscription.unsubscribe();
      ...
      if (client.prevJSONData != initPayload) {
        ...
        */
        ...
        if (initPayload['live_board']['top_of_inning']) {
          payload = '010000' + initPayload['live_board']['balls'] + initPayload['live_board']['strikes'] + initPayload['live_board']['outs'] + client.add_tens_zero(initPayload['live_board']['inning']) + client.add_tens_zero(initPayload['live_board']['away_score']) + client.add_tens_zero(initPayload['live_board']['home_score']) + 'T' + client.add_tens_zero(initPayload['live_board']['away_hits']) + client.add_tens_zero(initPayload['live_board']['home_hits']) + client.force_ones(initPayload['live_board']['away_errors']) + client.force_ones(initPayload['live_board']['home_errors']);
        } else {
          payload = '010000' + initPayload['live_board']['balls'] + initPayload['live_board']['strikes'] + initPayload['live_board']['outs'] + client.add_tens_zero(initPayload['live_board']['inning']) + client.add_tens_zero(initPayload['live_board']['away_score']) + client.add_tens_zero(initPayload['live_board']['home_score']) + 'B' + client.add_tens_zero(initPayload['live_board']['away_hits']) + client.add_tens_zero(initPayload['live_board']['home_hits']) + client.force_ones(initPayload['live_board']['away_errors']) + client.force_ones(initPayload['live_board']['home_errors']);
        }
        ...
        payload = payload.padEnd(133, ' '); // payload += '01743'; //Add BOT code
